...
const INITIAL_SLIDERS: SliderConfig[] = [
  { id: "tone1", label: "Tone", leftLabel: "Casual", rightLabel: "Professional", value: 70 },
  { id: "tone2", label: "Tone", leftLabel: "Casual", rightLabel: "Professional", value: 65 },
  { id: "tone3", label: "Tone", leftLabel: "Casual", rightLabel: "Professional", value: 72 },
];

const INITIAL_PROMPTS: Prompt[] = [
  { id: "1", label: "Prompt 1", text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nunc arcu, laoreet ut ornare ut, condimentum scelerisque nisl." },
  { id: "2", label: "Prompt 2", text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nunc arcu, laoreet ut ornare ut, condimentum scelerisque nisl." },
  { id: "3", label: "Prompt 3", text: "Make the tone more professional and concise." },
];

const EXAMPLE_CREATE_PROMPTS = [
  { label: "Prompt 1", text: "Write a short introduction paragraph about the history of renewable energy." },
  { label: "Prompt 2", text: "Make it professional and concise." },
];
...
function AiIcon({ size = 26 }: { size?: number }) {
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div style={{ position: "absolute", bottom: 0, right: 0, width: size * 0.42, height: size * 0.46 }}>
      ...
      >
        ...
        {!readOnly && (
          <div className="flex flex-col gap-0.5 ml-2">
            <button onClick={() => onChange?.(value + 1)} className="hover:opacity-70 transition-opacity leading-none">
              <ChevronUp size={12} color="#A6A6A6" />
            </button>
            <button onClick={() => onChange?.(Math.max(0, value - 1))} className="hover:opacity-70 transition-opacity leading-none">
...
export function AiToolPanel({ onClose, activeTab, onTabChange }: AiToolPanelProps) {
  ...
  const handleLabelKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") finalizeLabelEdit();
  };
  ...
  return (
    <>
      {showAdvanced && <AdvancedOptionsModal onClose={() => setShowAdvanced(false)} />}
      ...
      <div className="flex flex-col bg-[#303030] w-[370px] flex-shrink-0 h-full border-l border-[#898989]/20 relative">
        ...
        <div className="px-4 pt-4 pb-3 flex-shrink-0">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-[13px] transition-colors ${
                activeTab === "rewrite" ? "bg-[#3a3a3a] text-[#E9E9E9]" : "text-[#898989] hover:text-[#A6A6A6]"
              }`}
            >
              Rewrite
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-full text-[13px] transition-colors ${
                activeTab === "create" ? "bg-[#3a3a3a] text-[#E9E9E9]" : "text-[#898989] hover:text-[#A6A6A6]"
              }`}
            >
              Create
            </button>
          ...
          {/* Mode switcher + undo/redo — Rewrite only */}
          {activeTab === "rewrite" && (
            ...
            <div className="mx-4 mb-3 flex-shrink-0">
              ...
              <div className="flex items-center justify-between mt-2 px-1">
                ...
                <div className="flex items-center gap-1">
                  <button className="hover:opacity-70 transition-opacity"><RotateCcw size={15} color="#898989" /></button>
                  <button className="hover:opacity-70 transition-opacity p-1"><ChevronLeft size={15} color="#898989" /></button>
                  <span className="text-[#a6a6a6] text-[10px] mx-1">1/1</span>
                  <button className="hover:opacity-70 transition-opacity p-1"><ChevronRight size={15} color="#898989" /></button>
                </div>
              </div>
            </div>
              ...
              {mode === "slider" ? (
                <div className="flex flex-col gap-5 py-2">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="relative">
                          ...
                          {openMenuId === slider.id && (
                            <div ref={menuRef} className="absolute right-0 top-full mt-1 bg-[#262626] rounded-[6px] shadow-lg border border-[#555] py-1 z-10 min-w-[120px]">
                              <button
                                onClick={() => { deleteSlider(slider.id); setOpenMenuId(null); }}
                  ...
                  <div className="border-t border-[#555] pt-4 mt-1">
                    ...
                    >
                      <span className="text-[#E9E9E9] text-[14px] font-semibold">Advanced Options</span>
                      <ChevronDown size={16} color="#898989" className={`transition-transform duration-200 ${advancedExpanded ? 'rotate-180' : ''}`} />
                    </button>
                      ...
                      <div className="mt-4 flex flex-col gap-5 pb-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <button className="hover:opacity-70 transition-opacity"><HelpCircle size={14} color="#898989" /></button>
                          </div>
                          <div className="relative">
                            <input
                              type="range"
                        ...
                        <div className="flex items-center justify-between">
                          <p className="text-[#E9E9E9] text-[13px] font-medium">Use document spelling and grammar</p>
                          <button
                            ...
                            className={`w-5 h-5 rounded-[4px] border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                              useSpelling ? "bg-[#8149EC] border-[#8149EC]" : "bg-transparent border-[#898989] hover:border-[#A6A6A6]"
                            }`}
                          >
                            {useSpelling && (
                  ...
                  {prompts.map((prompt) => (
                    <div key={prompt.id} className="bg-[#262626] rounded-[8px] p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[#A6A6A6] text-[11px] font-medium">{prompt.label}</span>
                        <div className="flex items-center gap-2">
                          <button onClick={() => deletePrompt(prompt.id)} className="hover:opacity-70 transition-opacity">
                      ...
                      <p className="text-[#E9E9E9] text-[12px] leading-[1.5]">{prompt.text}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="flex flex-col gap-3">
                {createPrompts.length === 0 ? (
                  ...
                  <div className="opacity-40 pointer-events-none select-none flex flex-col gap-3">
                    <span className="text-[#A6A6A6] text-[11px] font-medium uppercase tracking-wide">Example</span>
                  ...
                  createPrompts.map((prompt) => (
                    <div key={prompt.id} className="bg-[#262626] rounded-[8px] p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[#A6A6A6] text-[11px] font-medium">{prompt.label}</span>
                        <div className="flex items-center gap-2">
                          <button onClick={() => deleteCreatePrompt(prompt.id)} className="hover:opacity-70 transition-opacity">
                            <Trash2 size={13} color="#E03030" />
                          </button>
                          <button className="hover:opacity-70 transition-opacity">
                            <Pencil size={13} color="#898989" />
